# These first few functions are copied from utils.py in the MathVista repository
import re
import os
import json
import base64
...
def extract_answer(response, problem, quick_extract=False):
    ...
    if quick_extract:
        ...
        result = re.search(r"The answer is\s?\(?([A-Z])\)?\.", response)
        if result:
            extraction = result.group(1)
            return extraction
        else:
          result = re.search(r"\(?([A-Z])\)?\.?", response)
          if result:
              extraction = result.group(1)
              return extraction
          else:
            print(f"Error in extracting answer for {problem['pid']} with answer type {problem['answer_type']}")
            return ""
        # except:
        #     pass
...
# Save Model Outputs
import pickle
with open('model_eval_1_8b.pkl', 'wb') as file:
